...
LCAH is a small local coding agent that turns one user request into a bounded
...
- `LCAH` owns session state, memory, tools, and workspace safety.
...
be validated through persisted `.lcah/sessions/*.json` and
`.lcah/sessions/*.events.jsonl` artifacts before treating a runtime change as
done.
...
- `.lcah/runs/<run_id>/task_state.json`
- `.lcah/runs/<run_id>/trace.jsonl`
- `.lcah/runs/<run_id>/report.json`
