...
class WelcomeBanner(Static):
    ...
    def render(self) -> Text:
        ...
        rows = [
            Text.assemble(
                Text("LCAH", style=f"bold {accent}"),
                Text("  local coding agent hub", style=muted),
            ),
            Text(""),
        ]
        rows.extend(Text(line, style=accent) for line in LCAH_MARK)
        rows.extend(
            [
                Text(""),
                Text.assemble(
...
class InputBar(Static):
    ...
    def __init__(self) -> None:
        super().__init__()
        self.input = Input(placeholder="Ask LCAH or type /help")
    ...
    def set_busy(self, busy: bool) -> None:
        ...
        self.input.placeholder = (
            "LCAH is working..." if busy else "Ask LCAH or type /help"
        )
