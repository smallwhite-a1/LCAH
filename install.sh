...
REPO="${LCAH_REPO:-https://github.com/smallwhite-a1/LCAH.git}"
INSTALL_DIR="${LCAH_INSTALL_DIR:-$HOME/.lcah-agent}"
BRANCH="${LCAH_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[lcah]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[lcah]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[lcah]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[lcah] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    BIN_DIR="${LCAH_BIN_DIR:-$HOME/.local/bin}"
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/lcah"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/lcah" "\$@"
EOF
    chmod +x "$LAUNCHER"
