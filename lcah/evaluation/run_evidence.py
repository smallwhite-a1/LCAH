"""Read-only helpers for LCAH run/session evidence.

These helpers intentionally do not import or instantiate the LCAH runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.lcah/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".lcah" / "runs")
        ...
        session_path = _latest_file(workspace / ".lcah" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".lcah" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
