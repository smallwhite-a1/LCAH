...
这个模块负责把“用户怎么启动 lcah”翻译成 runtime 能理解的对象：
...
from .tools.auto_skills import autoskill_status
...
DEFAULT_SECRET_ENV_NAMES = (
    ...
    "OPENAI_API_KEY",
    "OPENAI_API_TOKEN",
    "LCAH_ANTHROPIC_API_KEY",
    "ANTHROPIC_API_KEY",
    "ANTHROPIC_AUTH_TOKEN",
    "LCAH_DEEPSEEK_API_KEY",
    "DEEPSEEK_API_KEY",
    "LCAH_RIGHT_CODES_API_KEY",
    "RIGHT_CODES_API_KEY",
    "GITHUB_PAT",
    "GH_PAT",
)
...
WELCOME_NAME = "LCAH"
WELCOME_SUBTITLE = "local coding agent hub"
WELCOME_STATUS = "robot shell, ready for work"
HELP_DETAILS = (
    command_help_text()
    + "\n\n"
    + textwrap.dedent(
    ...
    /auto-skills Show project auto-skills created under .lcah/skills.
    """
    ).strip()
)
...
SECRET_ENV_NAMES_VAR = "LCAH_SECRET_ENV_NAMES"
...
def build_agent(args):
    ...
    - 输出：一个新的 `LCAH`，或一个从旧 session 恢复出来的 `LCAH`
    ...
    store = SessionStore(workspace.repo_root + "/.lcah/sessions")
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--config", default=None, help="Path to a LCAH TOML config file."
    )
    parser.add_argument(
        "--provider",
        default=None,
    ...
    parser.add_argument(
        "--memory-dir",
        default=None,
        help="Memory directory. Defaults to .lcah/memory in the workspace.",
    )
    parser.add_argument(
        "--no-auto-dream",
        action="store_true",
...
def handle_repl_command(agent, user_input):
    ...
    if user_input == "/auto-skills":
        return True, False, autoskill_status(agent)
...
def main(argv=None):
    ...
    if mode == "tui":
        from .tui.app import LCAHTuiApp
