...
SKILL_FILE_CREATION_GUIDE = """When creating LCAH skill files at .lcah/skills/<name>/SKILL.md or skills/<name>/SKILL.md, use frontmatter:
---
name: audit
...
def _auto_skill_blocked(path):
    status_path = Path(path).parent / ".autoskill.json"
    if not status_path.exists():
        return False
    try:
        metadata, _body = parse_frontmatter(Path(path).read_text(encoding="utf-8"))
        status_text = status_path.read_text(encoding="utf-8")
    except OSError:
        return False
    return metadata.get("source") == "auto" and '"status": "registered"' not in status_text
